...
bool do_exec_redirect(const char *outputfile, int count, ...)
{
    ...
    if (pid == 0) {
        ...
        if (fd < 0) {
            // If opening the file fails, exit
            return false;
        }
        ...
        // Close the file descriptor as it's no longer needed in the parent
        // We do not need to wait for the file operations, only the child process
        int status;
        if (waitpid(pid, &status, 0) == -1) {
            return false;
        }
